...
const removeParsingSign = (w: string) => w.replace(/[\u064B-\u0650]$/, ''); // حذف علامة الإعراب
const trimEndingYeh = (w: string) => w.replace(/\u064A$/g, diacs.kn); // حذف ياء الاسم المنقوص
const nominativeToAccusativePlr = (w: string) =>
  w.replace(/\u0648\u0646$/, '\u064A\u0646'); // نصب جمع المذكر السالم (أو جره)
const sign = (w: string, sign: string) =>
...
type Options = {
  ...
  isSngVaried?: boolean;
  isPlrVaried?: boolean;
  parsing?: 'nominative' | 'accusative' | 'genitive' | null;
  keepSign?: boolean;
  decimal?: number;
  before?: string;
  after?: string;
 ...
 * MAIN FUNCTION
 */
const parseDistinctive = (
  num: number,
  {
    sng = 'مرة', // صيغة المفرد
    dual = 'مرتين', // صيغة المثنى
    plr = 'مرات', // صيغة الجمع
  ...
  const twoResult =
    parsing === 'nominative'
      ? dual.replace(/[يا]ن$/, 'ان')
      : parsing === 'accusative' || parsing === 'genitive'
      ? dual.replace(/[يا]ن$/, diacs.f + 'ين')
      : dual;
...
export default parseDistinctive;
