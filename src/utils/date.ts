...
export const getDateNDaysAgo = (n: number) => {
  ...
  now.setDate(now.getDate() - Math.floor(n)); // subtract n days
...
export interface HijriOptions {
  ...
  locale?: string;
  calendar?: string;
  [key: string]: Intl.DateTimeFormatOptions[keyof Intl.DateTimeFormatOptions]; // For any additional Intl.DateTimeFormatOptions
}
...
export const hijriToCalendars = (
  ...
  let gD: Date = new Date(Date.UTC(2000, 0, 1));
  gD = new Date(
    gD.setUTCDate(
      gD.getUTCDate() +
  ...
  const gY: number = gD.getUTCFullYear() - 2000;
  ...
  function formatOutput(gD: Date): Date | string {
    return 'toCal' in op
